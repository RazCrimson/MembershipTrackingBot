...
class PermissionManagement(commands.Cog):
    ...
    async def on_command_error(self, ctx: Context, error):
        ...
        if isinstance(error, bot_exceptions.NotEnoughPerms):
            await ctx.send(f"You dont have enough permission to run that command | error:  {error}")
    ...
    async def authorize(self, ctx: Context, item: Union[discord.member.User, discord.guild.Role], level: int):
        ...
        role_server_id = 0
        if level > 10:
            await ctx.send('You are attempting to give a permission higher than the max')
            return
        ...
        if target_level is None:
            ...
            if target_level >= self_level != 10:
                await ctx.send('Target has a higher or equal clearance level!')
            else:
    ...
    async def test(self, ctx: Context, mention: Union[discord.TextChannel, str]):
        await ctx.send(f"Content is:  {mention},  Type:  {type(mention)}")
